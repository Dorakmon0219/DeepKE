...
def add_args():
    ...
    parse.add_argument("--input_file", type=str, default="train.json", help="input file path")
    parse.add_argument("--output_file", type=str, default="train_input.json", help="output file path")
    parse.add_argument("--number", default=5, help="number of examples")
    parse.add_argument("--mode", default="train", choices=["train", "test"])
...
def convert_train(options):
    records = []
    with open(options.input_file, "r",encoding='utf-8') as reader:
        for line in reader:
